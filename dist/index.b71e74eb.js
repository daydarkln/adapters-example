...
var _localStorageAdapter = require("./adapters/localStorage.adapter");
// import { restAdapter } from "./adapters/rest.adapter";
const store = (0, _localStorageAdapter.localStorageAdapter)();
// const store = restAdapter();
store.createRecord("type", "[1,2,3,4,5,6]");

},{"./adapters/localStorage.adapter":"hmFRU"}],"hmFRU":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "localStorageAdapter", ()=>localStorageAdapter);
const localStorageAdapter = ()=>{
    if (!localStorage) throw new Error("localStorage not awailable");
    return {
        findRecord (type, id) {
        ...
        deleteRecord (type, id) {
            ...
            localStorage.setItem(type, JSON.stringify(t.filter((item)=>item.id !== id)));
            return Promise.resolve(JSON.parse(localStorage.getItem(type) || ""));
        },
        updateRecord (type, id, payload) {
            const t = JSON.parse(localStorage.getItem(type) || "");
            localStorage.setItem(type, JSON.stringify(t.map((item)=>item.id === id ? payload : item)));
            return Promise.resolve(JSON.parse(localStorage.getItem(type) || ""));
        },
...
},{}]},["iJYvl","h7u1C"], "h7u1C", "parcelRequire5a6e")
